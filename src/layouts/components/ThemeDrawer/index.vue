<template>
	<el-drawer v-model="drawerVisible" title="布局设置" size="300px">
		...
		<div class="theme-item">
			<span>主题颜色</span>
			<el-color-picker v-model="themeConfig.primary" :predefine="colorList" @change="changePrimary($event as string)" />
		</div>
		<div class="theme-item">
			<span>暗黑模式</span>
			<SwitchDark />
...
import { mittBus, OPEN_THEMEDRAWER } from "@/utils/mittBus";
import { DEFAULT_PRIMARY } from "@/config/config";
import SwitchDark from "@/components/SwitchDark/index.vue";

// 打开主题设置
const drawerVisible = ref(false);
mittBus.on(OPEN_THEMEDRAWER, () => (drawerVisible.value = true));
...
// 监听布局配置变化，在 body 上添加相对应的 layout class
watch(
