...
const columns: Partial<ColumnProps>[] = [
	...
	{ prop: "username", label: "用户姓名", width: 130, searchable: true, searchElComponentProps: { disabled: true }, renderHeader },
	...
	// 😄 enum 为请求方法时，后台返回的数组对象 key 值不是 label 和 value 的情况，可以在 searchProps 中指定 label 和 value 的 key 值
	{
		...
		searchType: "select",
		enum: getUserGender,
		searchElComponentProps: { label: "genderLabel", value: "genderValue" }
	},
	{ prop: "idCard", label: "身份证号", searchable: true },
	{ prop: "email", label: "邮箱", searchable: true },
	{ prop: "address", label: "居住地址", searchable: true },
	{
		prop: "status",
		label: "用户状态",
		sortable: true,
		...
		searchType: "select",
		enum: getUserStatus,
		searchElComponentProps: { label: "userLabel", value: "userStatus" }
	},
	{
		prop: "createTime",
		label: "创建时间",
