	...
	<div class="table-box">
		<div class="table-search" v-show="isShowSearch">
			<el-form ref="formRef" :model="searchParam" :inline="true" label-width="100px">
				...
				<div class="more-item" v-show="searchMoreShow">
			...
			<div class="search-operation">
				...
				<el-button type="primary" link class="search-isOpen" @click="searchMoreShow = !searchMoreShow">
					{{ searchMoreShow ? "合并" : "展开" }}
					<el-icon class="el-icon--right">
						<component :is="searchMoreShow ? ArrowUp : ArrowDown"></component>
					</el-icon>
				</el-button>
			</div>
		</div>
		...
		<el-table :data="tableData" :border="true" @selection-change="selectionChange" :row-key="getRowKeys">
			...
			<el-table-column prop="username" label="用户姓名" :formatter="defaultFormat" show-overflow-tooltip width="140" />
			<el-table-column prop="gender" label="性别" show-overflow-tooltip width="140" v-slot="scope">
				{{ scope.row.gender == 1 ? "男" : "女" }}
			</el-table-column>
			<el-table-column prop="idCard" label="身份证号" :formatter="defaultFormat" show-overflow-tooltip />
			<el-table-column prop="email" label="邮箱" :formatter="defaultFormat" show-overflow-tooltip width="240" />
			<el-table-column prop="address" label="居住地址" :formatter="defaultFormat" show-overflow-tooltip />
			<el-table-column prop="createTime" label="创建时间" :formatter="defaultFormat" show-overflow-tooltip width="200" />
			...
			<el-table-column label="操作" fixed="right" width="330" v-slot="scope">
				<el-button v-if="BUTTONS.view" type="primary" link :icon="View" @click="openDrawer('查看', scope.row)">查看</el-button>
				<el-button v-if="BUTTONS.edit" type="primary" link :icon="EditPen" @click="openDrawer('编辑', scope.row)">编辑</el-button>
				<el-button v-if="BUTTONS.reset" type="primary" link :icon="Refresh" @click="resetPass(scope.row)">重置密码</el-button>
